"""
train_traffic_light_export_v3.py

Versão melhorada para classificar semáforo red / green com modelo linear exportável
mais forte, usando:
- crop central mais agressivo
- normalização de iluminação
- features de cor específicas para semáforo
- busca de hiperparâmetro C no LogisticRegression
...
Uso:
python train_traffic_light_export_v3.py --dataset ./dataset --out traffic_light_model.h --samples 8
"""
...
def extract_features(img_rgb: np.ndarray) -> np.ndarray:
    ...
    feats.extend([
        ft[0] - fb[0],    # R topo - base
        ft[1] - fb[1],    # G topo - base
        ft[7] - fb[7],    # red_ratio topo - base
        ft[8] - fb[8],    # green_ratio topo - base
        ft[9] - fb[9],    # red_strength topo - base
        ft[10] - fb[10],  # green_strength topo - base
        ft[11] - fb[11],  # red_peak topo - base
        ft[12] - fb[12],  # green_peak topo - base
    ])
...
def train_and_quantize(dataset_dir: Path, test_size: float = 0.25, random_state: int = 42) -> ExportBundle:
    ...
    for C in [0.03, 0.05, 0.1, 0.3, 1.0, 3.0, 10.0]:
...
def emit_header(bundle: ExportBundle, out_path: Path, num_samples: int = 8) -> None:
    ...
    lines.append("")
    lines.append("// Gerado automaticamente por train_traffic_light_export_v3.py")
